...
@allure.feature('购物车功能') # featured定义功能
class TestShoppingTrolley(object):
    ...
    def test_add_shopping_trolley(self):
        ...
        with allure.step("点击商品"): # 讲一个测试用例分成几个步骤，将步骤打印到测试报告中，步骤3
            pass
...
@allure.step('用户登录') # 还可以将一个函数座位一个步骤，调用此函输时，报告中输出一个步骤，步骤名字通常是函数名，我把这样的函数叫“步骤函数
def login(user, pwd):
    print(user, pwd)